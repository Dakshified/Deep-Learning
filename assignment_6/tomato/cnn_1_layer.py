...
# Define the CNN architecture with only 1 Conv layer
class CNN1Layer(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.fc = nn.Linear(16 * 128 * 128, num_classes) # Adjusted to 128x128 image size to match your settings
...
def main():
    print("="*60)
    print("Training 1-Layer CNN in PyTorch (Conv -> ReLU -> Flatten -> Linear)")
    print("="*60)
    ...
    model = CNN1Layer(num_classes=num_classes).to(device)
    print(model)
    ...
    weights_path = os.path.join(base_dir, 'cnn_1_layer_weights.pth')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'cnn_1_layer_metrics.png'))
    plt.close()
