...
# Define the CNN architecture with Conv and Pooling
...
def main():
    print("="*60)
    print("Training CNN with Max Pooling in PyTorch (Conv -> MaxPool -> Flatten -> Linear)")
    print("="*60)
    ...
    model = CNNPooling(num_classes=num_classes).to(device)
    print(model)
    ...
    weights_path = os.path.join(base_dir, 'cnn_pooling_weights.pth')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, 'cnn_pooling_metrics.png'))
    plt.close()
